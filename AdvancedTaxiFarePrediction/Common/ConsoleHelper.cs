...
namespace Common
{
    public static class ConsoleHelper
    {
        ...
        static decimal win = 0;
        static decimal loss = 0;
        static  decimal tot = 0;
        static decimal wint = 0;
        public static void PrintRegressionPredictionVersusObserved(string predictionCount, string observedCount)
        {
            ...
            if (float.Parse(predictionCount) > 0.95)
            {
                if (float.Parse(observedCount) > 0.8)
                    win++;
                else
                    loss++;
               tot = win + loss;
               wint = decimal.Divide(win, tot);
            }
            
            Console.WriteLine($"Win:{win.ToString()} Loss:{loss.ToString()} Total:({tot})  Win%:{wint:0.##}");
            Console.WriteLine($"-------------------------------------------------");
        }
